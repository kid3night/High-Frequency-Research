...
for path in data_dict_1:
	...
	if feature_name in caled_list:
		...
		for period in data_dict_1[path]:
			...
			feature_list1.append(temp_data.values[:25300981].reshape(-1, 1).copy())
			feature_name_list1.append(feature_name + '_' + period)
			del temp_data
			gc.collect()
			del gc.garbage[:]
			gc.collect()
...
design_matrix = np.concatenate(feature_list1, axis=1)
feature_names_array = pd.Series(np.array(feature_name_list1))
df_feature = pd.DataFrame(design_matrix, columns=feature_names_array)

df_feature.to_hdf('feature_matrix_test_set.h5', key='raw_feature')
finite_pos = np.isfinite(df_feature).all(axis=1)
finite_pos.to_hdf('feature_matrix_test_set.h5', key='finite_pos')
df_feature_clean = df_feature.loc[finite_pos, :]
df_feature_clean.to_hdf('feature_matrix_test_set.h5', key='clean_feature')
pd.Series(df_feature_clean.columns.values).to_hdf('feature_matrix_test_set.h5', key='columns_names')