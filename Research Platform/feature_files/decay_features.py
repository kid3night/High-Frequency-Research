...
class Transaction_Cancellation_Square_decay(base_feature):
	param_list = ['nperiod', 'decay_factor']
	...
	def feature(self, data_fed, nperiod, decay_factor):
		...
		def function_inside(data, decay_factor):
			if len(data) == 0:
				...
				decay_weight = decay_within_ticks(len(data), decay_factor)
				net_sum = (data * decay_weight).sum() ** 2
				return np.log(net_sum) if net_sum > 0 else -np.log(-net_sum) if net_sum < 0 else 0
...
class tran_continuous_elements_num(base_feature):
	param_list = ['nperiod']
	...
	def feature(self, data_fed, nperiod):
		...
		def function_inside(data):
			if len(data) == 0:
				...
				res_list = np.split(data, np.where(np.diff(data) != 0)[0] + 1)
				res_sum = [i.sum() for i in res_list]
				max_num = max(res_sum)
				min_num = min(res_sum)
				buy_order = max_num if max_num > 0 else 0
				sell_order = min_num if min_num < 0 else 0
				return buy_order + sell_order

		rolling_time = str(nperiod * 3) + 'S'
		bs_multiplier = np.where(stk_data['BSFlag'].values == 66, 1, np.where(stk_data['BSFlag'].values == 83, -1, 0))
		sign_rolling_num = pd.Series(bs_multiplier, index=stk_data['BSFlag'].index)
		return sign_rolling_num.rolling(rolling_time,  closed='left').apply(function_inside, raw=True)


class tran_continuous_elements_volume(base_feature):
	param_list = ['nperiod']
	...
	def feature(self, data_fed, nperiod):
		...
		def function_inside(data):
			if len(data) == 0:
				...
				res_list = np.split(data, np.where(np.diff(sign_data) != 0)[0] + 1)
				len_list = np.split(sign_data, np.where(np.diff(sign_data) != 0)[0] + 1)
				res_sum = [i.sum() for i in len_list]
				longest_buy_pos = np.argmax(res_sum)
				max_buy_pos = np.max(res_sum)
				longest_sell_pos = np.argmin(res_sum)
				max_sell_pos = np.min(res_sum)
				buy_order_vol = np.sum(res_list[longest_buy_pos]) if max_buy_pos > 0 else 0
				sell_order_vol = np.sum(res_list[longest_sell_pos]) if max_sell_pos < 0 else 0
				return buy_order_vol + sell_order_vol

		rolling_time = str(nperiod * 3) + 'S'
		bs_multiplier = np.where(stk_data['BSFlag'].values == 66, 1, np.where(stk_data['BSFlag'].values == 83, -1, 0))
		sign_rolling_vol = stk_data['Volume'] * bs_multiplier
		return sign_rolling_vol.rolling(rolling_time,  closed='left').apply(function_inside, raw=True)


class order_continuous_elements_volume(base_feature):
	param_list = ['nperiod']
	...
	def feature(self, data_fed, nperiod):
		...
		sign_rolling_amount = stk_data['Volume'] * bs_multiplier
		return sign_rolling_num.rolling(rolling_time,  closed='left').apply(function_inside, raw=True)


class order_continuous_elements_num(base_feature):
	param_list = ['nperiod']
	...
	def feature(self, data_fed, nperiod):
		...
		rolling_time = str(nperiod * 3) + 'S'
		bs_multiplier = np.where(stk_data['FunctionCode'].values == 66, 1, np.where(stk_data['FunctionCode'].values == 83, -1, 0))
		sign_rolling_num = pd.Series(bs_multiplier, index=stk_data['FunctionCode'].index)
		return sign_rolling_num.rolling(rolling_time,  closed='left').apply(function_inside, raw=True)